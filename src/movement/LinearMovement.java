...
public class LinearMovement extends MovementModel {
	...
	private Queue<Coord> pass=new LinkedList<>();
	...
	public LinearMovement(LinearMovement ilm) {
		...
		if (ilm.targetType == 0) { /* random target */
			...
			for (int i=0; i<5000;i++) {
				...
				this.nextPath.addWaypoint(calculateLocation(ilm.endLoc, ilm.startLoc, initLoc));
				this.nextPath.addWaypoint(calculateEndTarget(ilm, initLoc));
	...
	private Coord calculateLocation(Coord endLoc,Coord startLoc, Coord initLoc) {
		return (startLoc.distance(initLoc) <
				endLoc.distance(initLoc) ? startLoc: endLoc);
	}
