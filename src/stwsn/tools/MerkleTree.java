...
public class MerkleTree {
	...
	public MerkleTree(List<String> leafSignatures) {
		treeStep=new ArrayList<List<Node>>();
		constructTree(leafSignatures);
	}
	
	public MerkleTree(String leaf,String Otherleaf,List<byte[]> siblingValues) {
		constructTreeFromSib(leaf,Otherleaf,siblingValues);
	}
	...
	private void constructTreeFromSib(String leaf, String otherleaf,List<byte[]> siblingValues) {
		String[] values=new String[2];
		values[0]=leaf;
		values[1]=otherleaf;
		List<Node> parents=bottomLevel(Arrays.asList(values));
		for (int i=0; i<siblingValues.size();i++){
			parents.add(constructInternalNodeFromSibling(parents.get(0),siblingValues.get(i)));
		}
		root = parents.get(0);
	 ...
	 * @param signatures
	 */
	public byte[][] getSiblings(int leafNumber) {
		int i=0;
		leafNumber=leafNumber-1;
		List<Node> parents = treeStep.get(i++);
		byte[][] sib=new byte[depth-1][];
		int sibindex=this.NextSib(leafNumber,parents.size());
		int s=0;
		while (s<sib.length) {
			sib[s++]=parents.get(sibindex).sig;
			parents = treeStep.get(i++);
			sibindex=NextSib(sibindex,parents.size());	
		}
		return sib;
	}

	public String getBrotherLeaf(int leafNumber){
		int sibindex=(leafNumber%2==0) ? leafNumber+1 : leafNumber-1;;
		return leafSigs.get(sibindex%leafSigs.size());
	}
	
	public String getLeafSign(int leafNumber){
		if (leafNumber==leafSigs.size())
			leafNumber=leafNumber-1;
		return leafSigs.get(leafNumber);
	}



	private  int NextSib(int index,int arraylength){
		int Next=(int) Math.ceil((index-1)/2);
		//if (arraylength%2==0){
		int v= (Next%2==0) ? Next+1 : Next-1;
		if (v<0) v=0;
		if (v==arraylength)v=v-1;
		return v;
		//}
		//return (Next%2==0) ? Next-1 : Next+1;
	...
	private Node constructInternalNodeFromSibling(Node child1, byte[] child2) {
		Node parent = new Node();
		parent.type = INTERNAL_SIG_TYPE;
		parent.sig = internalHash(child1.sig,child2);
		parent.left = child1;
		return parent;
